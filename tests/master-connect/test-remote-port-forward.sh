...
# Test remote port forwarding (-R): remote port 9999 forwards to local port 8888
# This test verifies that the remote port forwarding setup works correctly

# Start goncat with remote port forwarding
...
send -- "echo 'Connected with remote port forwarding enabled'\n"
Utils::wait_for "Connected with remote port forwarding enabled"
...
send -- "echo 'REMOTE_PORT_FORWARD_TEST_SERVER' | nc -l -p 8888 &\n"
send -- "sleep 1\n"
Utils::wait_for "# " 0
...
send -- "echo 'Verifying remote port forwarding configuration...'\n"
Utils::wait_for "Verifying remote port forwarding configuration"
...
send -- "echo 'REMOTE_CONNECT_TEST' | nc -w 2 127.0.0.1 8888 || echo 'Remote connection attempt completed'\n"
Utils::wait_for "Remote connection attempt completed"

# If we reach here, the remote port forwarding setup is working
send -- "echo 'SUCCESS: Remote port forwarding test passed'\n"
Utils::wait_for "SUCCESS: Remote port forwarding test passed"
